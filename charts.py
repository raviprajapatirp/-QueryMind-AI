...
except ImportError:  # pragma: no cover - charts.py can still be imported standalone
    st = None

# "Notebook Desk" colorway: pencil-indigo, highlighter-coral, correct-green,
# star-gold, ink-navy, sky-violet — friendly and legible on a light background.
COLORWAY = ["#5B5FEF", "#FF7A59", "#21C17C", "#FFB020", "#1E2A55", "#8B7BFF", "#FF5470"]


def _current_template() -> str:
    if st is not None and st.session_state.get("theme") == "dark":
        return "plotly_dark"
    return "plotly_white"
...
def _apply_layout(fig, title=""):
    fig.update_layout(
        ...
        title_font=dict(family="'Baloo 2', 'Trebuchet MS', sans-serif", size=20),
        colorway=COLORWAY,
